...
public class AigcDocsSliceController {
    ...
    @GetMapping("/list")
    public CommonResponse<List<AigcDocsSlice>> list(AigcDocsSlice data) {
        return CommonResponse.ok(docsSliceMapper.selectList(Wrappers.<AigcDocsSlice>lambdaQuery().orderByDesc(AigcDocsSlice::getCreateTime)));
    }

    @GetMapping("/page")
    public CommonResponse list(AigcDocsSlice data, QueryPage queryPage) {
        Page<AigcDocsSlice> page = new Page<>(queryPage.getPage(), queryPage.getLimit());
        return CommonResponse.ok(MybatisUtil.getData(docsSliceMapper.selectPage(page, Wrappers.<AigcDocsSlice>lambdaQuery()
    ...
    @GetMapping("/{id}")
    public CommonResponse<AigcDocsSlice> findById(@PathVariable String id) {
        return CommonResponse.ok(docsSliceMapper.selectById(id));
    }

    @PostMapping
    @ApiLog("新增切片数据")
    @SaCheckPermission("aigc:docs:slice:add")
    public CommonResponse add(@RequestBody AigcDocsSlice data) {
    ...
    public CommonResponse update(@RequestBody AigcDocsSlice data) {
